...
CREATE TABLE IF NOT EXISTS `mydb`.`Recipe` (
  ...
  `recipe_type` ENUM('μαγειρική', 'ζαχαροπλαστική') NOT NULL,
  `National_Cuisine` VARCHAR(100) NOT NULL,
  `portions` VARCHAR(45) NOT NULL,
  `Image_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Steps` (
  ...
  `Image_id` INT NOT NULL,
  PRIMARY KEY (`step_id`),
  INDEX `fk_Steps_Image1_idx` (`Image_id` ASC) VISIBLE,
  CONSTRAINT `fk_Steps_Image1`
    FOREIGN KEY (`Image_id`)
    REFERENCES `mydb`.`Image` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `mydb`.`Recipe_has_Steps` (
  ...
  PRIMARY KEY (`Recipe_name`, `Steps_step_id`),
  INDEX `fk_Recipe_has_Steps_Steps1_idx` (`Steps_step_id` ASC) VISIBLE,
  INDEX `fk_Recipe_has_Steps_Recipe1_idx` (`Recipe_name` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Cook` (
  ...
  `professional_qualification` ENUM('Α΄ μάγειρας', 'Β΄ μάγειρας', 'Γ΄ μάγειρας', 'βοηθός
αρχιμάγειρα', 'αρχιμάγειρας') NOT NULL,
  `specialisation` VARCHAR(45) NOT NULL,
  `role` ENUM('κριτής', 'διαγωνιζόμενος ') NOT NULL,
  `Image_id` INT NOT NULL,
  PRIMARY KEY (`cook_id`),
  INDEX `cook_name_index` (`cook_first_name` ASC, `cook_last_name` ASC) INVISIBLE,
  INDEX `cook_age_index` (`age` ASC) VISIBLE,
  INDEX `cook_prof_qualification_index` (`professional_qualification` ASC) VISIBLE,
  INDEX `role_index` (`role` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Cook_has_National_Cuisine` (
  ...
  INDEX `fk_Cook_has_National_Cuisine_Image1_idx` (`Image_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Cook_has_National_Cuisine_National_Cuisine1`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Cook_has_National_Cuisine_Image1`
    FOREIGN KEY (`Image_id`)
    REFERENCES `mydb`.`Image` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
