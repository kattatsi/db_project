...
CREATE TABLE IF NOT EXISTS `mydb`.`Steps` (
  ...
  PRIMARY KEY (`step_number`, `Recipe_name`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`Nutrition_Info` (
  ...
  `calories_per_portion` INT NULL DEFAULT 0,
...
BEGIN
    ...
    FROM Cook c
    WHERE c.position_rank IN ('Head chef')                           /*changed to: position_rank, 'Head chef' */
    ORDER BY RAND()
    LIMIT 3;
